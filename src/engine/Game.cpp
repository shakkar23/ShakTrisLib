...
std::vector<Piece> Game::get_possible_piece_placements() const {
    ...
    std::vector<Piece> valid_pieces = Shaktris::MoveGen::movegen(srs_rotate, board, current_piece.type);

    PieceType holdType = hold.has_value() ? hold.value() : queue.front();
    if (holdType != PieceType::Empty) {
        std::vector<Piece> hold_pieces = Shaktris::MoveGen::movegen(srs_rotate, board, holdType);
